                ...
                if (window.Sentry) {
                    window.Sentry.init({
                        ...
                        beforeSend: function(event){
                            ...
                            if(window.parent === window && !event.request.url.match(/(https?:\/\/.*jotform\.com)|(https?:\/\/cdn\.jotfor\.ms)/)){
                                return undefined;
                            }

                            if(/FB_IAB || FBAV || FB4A/.test(window.navigator.userAgent)) {
                                return undefined;
                            }
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    setFocusEvents: function () {
        ...
        if (JotForm.newDefaultTheme && window.paymentType === 'product') {
        ...
        if (JotForm.newDefaultTheme && window.paymentType === 'subscription') {
    ...
    handleWidgetStyles: function () {
        ...
        if (JotForm.newDefaultTheme) {
            var styleTags = JotForm.collectStylesheet().filter(function(tag) {
                return tag.indexOf('/themes/CSS') > -1;
            });
               ...
               for (var inputId in combinedObject) {
                    if (inputId !== "") {
                        if ($('id_' + result).select('input[id*=' + inputId + '], select[id*=' + inputId + ']').length > 0) {
                            ...
                            if(resultFieldType == 'address' &&  window.FORM_MODE == 'cardform' && parentfieldInputElement){
                              parentfieldInputElement.value = combinedObject[inputId]; 
                            }
    ...
    initDateV2Inputs: function() {
        ...
        for(var i = 0; i < liteModeInputs.length; i++) {
            try {
                var imask = new Inputmask({
                    alias: 'datetime',
                    inputFormat: liteModeInputs[i].placeholder,
                    jitMasking: true,
                });
        ...
        var validatorEvent = function (e) {
            setTimeout(function() { // to let focus event to work
              try {
                ...
                if (inputContainer.dataset.type === 'control_fullname' && isPrefix && JotForm.newDefaultTheme) {
                    input.validateInput();
                }
